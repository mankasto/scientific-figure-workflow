...
- `structure_lock.yaml`: approved macro topology, visual center, relative region geometry, connector grammar, density target, and permitted structural deviation;
- `structure_skeleton.png`: rendered grayscale skeleton used to review the macrostructure before image generation;
...
When a reference exists, analyze its structure before writing the rendering prompt. Default to `preserve-reference` when the user asks to improve quality, editability, palette, typography, spacing, or imagery. Preserve the reference's visual center, topology, relative region placement, connector grammar, and information-density pattern. A structural redesign requires an explicit reason recorded in `review.md` and a revised skeleton. Do not advance the prompt gate until the structure lock and skeleton agree with the semantic inventory.
